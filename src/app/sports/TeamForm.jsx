  ...
  return (
    <form onSubmit={submit} style={{maxWidth:720}}>
      <div style={{marginBottom:12}}>
        ...
        <input value={name} onChange={e=>setName(e.target.value)} required style={{width:'100%',padding:8,borderRadius:4}} />
      </div>
      <div style={{marginBottom:12}}>
        <label style={{display:'block',marginBottom:4}}>Branch</label>
        <select value={branch} onChange={e=>setBranch(e.target.value)} style={{width:'100%',padding:8,borderRadius:4}}>
      ...
      <div style={{marginBottom:12}}>
        ...
        <select value={sportId} onChange={e=>setSportId(e.target.value)} required style={{width:'100%',padding:8,borderRadius:4}} disabled={loading}>
      ...
      <div style={{marginBottom:12}}>
        ...
        <button type="button" onClick={addMemberId} style={{padding:'6px 12px',marginBottom:8}}>Add member by id</button>
        <ul style={{listStyle:'none',padding:0}}>
          {memberIds.map(m => (
            <li key={m} style={{display:'flex',justifyContent:'space-between',padding:6,border:'1px solid #ddd',marginBottom:4,borderRadius:4}}>
              <span>User ID: {m}</span>
              <button type="button" onClick={()=>removeMemberId(m)} style={{cursor:'pointer'}}>Remove</button>
            </li>
          ))}
        </ul>
      </div>
      <div style={{marginBottom:12}}>
        ...
        <input value={captainId} onChange={e=>setCaptainId(e.target.value)} placeholder="Leave empty for manager to be captain" style={{width:'100%',padding:8,borderRadius:4}} />
      </div>
      <div>
        <button type="submit" style={{padding:'10px 20px',cursor:'pointer'}}>{submitLabel}</button>
      </div>
    </form>
  )
}
